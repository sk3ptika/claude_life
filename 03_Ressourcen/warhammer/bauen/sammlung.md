---
titel: Sammlung
typ: notiz
aktualisiert: 2026-08-16
---

# Sammlung

Eine Zeile je Einheit. Status: ungeöffnet → gebaut → grundiert → in Arbeit → bemalt → basiert → versiegelt.
Nur `versiegelt` zählt als fertig.

## Warhammer 40.000

| Fraktion | Einheit | Modelle | Status | Rezept | Gekauft | Notiz |
|---|---|---|---|---|---|---|
|  |  |  |  |  |  |  |

## Kill Team

| Team | Modelle | Status | Rezept | Gekauft | Notiz |
|---|---|---|---|---|---|
|  |  |  |  |  |  |

## Farben und Werkzeug

| Kategorie | Bestand | Nachkaufen |
|---|---|---|
| Grundierung schwarz | Chaos Black (Spray) | — |
| Grundierung grau/weiß | Mechanicus Standard Grey, Wraithbone (Sprays) | — |
| Grundierung rot | Mephiston Red (Spray) | — |
| Nasspalette | Army Painter | — |
| Pinsel Gr. 1 / Gr. 0 | Anzahl und Größen noch erfassen | — |
| Mattlack | fehlt | ja, in Deutschland — ohne ihn zählt kein Modell als fertig |
| Basing-Material | fehlt | ja, in Deutschland |
| Seitenschneider | noch erfassen | GodHand SPN-120, Japan 11/2026 |
| Schleifmittel | noch erfassen | GodHand Kamiyasu Schwämme, Japan 11/2026 |
| Handbohrer | fehlt | Tamiya Pin Vice, Japan 11/2026 |
| Maskierband | fehlt | Tamiya Präzisionsband, Japan 11/2026 |

Die vier mit „Japan 11/2026" markierten Positionen werden auf der Japan-Reise gekauft, weil sie dort deutlich günstiger sind — Details in [Gunpla-Einkauf Ōsaka](../../japan/gunpla-einkauf-osaka.md). Bis dahin nichts davon in Deutschland nachkaufen. Kleber, Lösungsmittel und Sprays kommen **nicht** aus Japan mit (Gefahrgut im Flug).

## Kennzahlen

Beim Aktualisieren mitführen:

- Modelle gesamt: 
- Davon versiegelt: __ (__ %)
- Zugänge letzte 90 Tage: 
- Fertiggestellt letzte 90 Tage: 

Wenn Zugänge dauerhaft über Fertigstellungen liegen, liegt das Problem beim Einkauf, nicht beim Pinsel.
