...
## Stand zu diesem Zeitpunkt (2026-08-17, Commit 9ac941d + Struktur-Übersicht)
...
**Ergänzung vom selben Tag, nach dem ersten Stand:** die Ordneransicht (`ansichtOrdner` in `app.js`, jetzt 2.969 Zeilen `_system/ui/` gesamt) zeigt jetzt bei Ordnern mit direkten Unterordnern eine „Struktur"-Kachelreihe statt der reinen Chip-Zeile — pro Unterordner rekursive Notizzahl, die eigenen Unterordner als Chips (zweite Ebene ohne Klick sichtbar) und eine Vorschau der zuletzt geänderten Notizen, leere Zweige zeigen „Noch leer.". Auslöser: `03_Ressourcen/warhammer/` hat bereits eine zweistufige Struktur (bauen · lore · malen/rezepte · spielen/40k+kill-team), die vorher nur als flache Dateiliste plus eine Ebene Chips sichtbar war. Die Notizenliste zeigt seitdem nur noch direkte Dateien des jeweiligen Ordners, nicht mehr alle verschachtelten — die liegen jetzt in den Struktur-Kacheln. Zusätzlich klickbare Brotkrumen im Seitenkopf für tiefe Pfade. `_anhaenge`-Unterordner werden aus der Struktur-Übersicht ausgeblendet (Systemkonvention für Anhänge, keine Themenstruktur — Inhalt erscheint weiter in der Dateien-Sektion). Rein aus `S.index.ordner`/`S.index.dateien` abgeleitet, kein Themenname im Code — gilt automatisch auch für `03_Ressourcen/fotografie/rezepte/`, das noch leer ist. Geprüft: Syntax via JavaScriptCore (`osascript -l JavaScript`, kein Node auf diesem Rechner installiert), Struktur-Zählungen für warhammer/fotografie/02_Bereiche/01_Projekte gegen den echten `/api/tree` nachgerechnet (Python, nur lesend). **Browser-Interaktionstest steht weiterhin aus.** In dieser Umgebung gibt es kein Browser-Automatisierungswerkzeug (kein Playwright/Puppeteer), nur HTTP-Anfragen und Datenabgleich — Klicks, Dunkelmodus und Schmalgerät wurden also nicht visuell geprüft, nur die zugrunde liegende Logik gegen echte Vault-Daten nachgerechnet. Commit erfolgte trotzdem auf ausdrücklichen Wunsch des Nutzers. Die Testliste unten bei Gelegenheit von Hand im Browser durchgehen, besonders Punkt 8 (Schmalgerät) für die neuen Struktur-Kacheln.

**Zweite Ergänzung, nach dem Klicktest:** der Klicktest bestätigte die Ressourcenansicht, zeigte aber erwartungsgemäß keine Änderung beim Öffnen einer Bereichsnotiz (`02_Bereiche/<slug>/_bereich.md`) — die Struktur liegt ja in `03_Ressourcen/<slug>/`, nicht im Bereichsordner selbst, der nur die eine Notiz enthält. Auf Nachfrage entschieden: `ansichtDoc` bekommt eine neue Feldrolle (kein Themenname) — bei `typ: bereich` unterhalb von `02_Bereiche/` wird geprüft, ob ein gleichnamiger Ordner `03_Ressourcen/<slug>/` existiert; falls ja und falls er Inhalt hat, erscheint unter dem Dokumenttext ein Abschnitt „Ressourcen-Struktur" mit denselben Struktur-Kacheln wie in der Ordneransicht, plus Chips für Dateien, die direkt im Ressourcenordner liegen (z. B. `fotografie/kamera-x100v.md`). Namensgleichheit zwischen Bereichs- und Ressourcenordner ist bereits an anderer Stelle (`bereicheSammeln`, Dashboard-Kachel-Matching) die etablierte Konvention, hier nur wiederverwendet. Geprüft an den echten Daten: `warhammer` (Bereich + Ressourcen vorhanden → 4 Kacheln), `fotografie` (Bereich + Ressourcen vorhanden → 2 Datei-Chips + 1 Kachel „rezepte"), `japan` (Ressourcen ohne Bereich → keine Auswirkung, Guard greift nicht), `finanzen`/`gesundheit` (Bereich ohne Ressourcen → Abschnitt bleibt aus, Doc-Ansicht unverändert). Auch hier: kein Browser-Klicktest, nur Datenabgleich — vor dem nächsten Weekly Review im Browser gegenprüfen.

**Dritte Ergänzung (Farbpalette, interaktive Tabellen & Google-Sheet-Sync):** Die Farbpalette (`1pttp6ck1cEQznE3pJk9kd88DyKdda0_C-7E5Be1zvaw`) wurde als versionierte Markdown-Ressource `03_Ressourcen/warhammer/malen/farbpalette.md` angelegt, die veraltete `.gsheet`-Verknüpfung aktualisiert und die Referenzen in `_bereich.md` sowie `projekte-bemalen.md` gepflegt. `_system/ui/` wurde generisch erweitert (kein Themenname im Code): Dokumente mit tabellarischen Daten (`tabelle_interaktiv: true` oder `gsheet_id`/`gsheet_url`) erhalten in `ansichtDoc` eine Toolbar mit Live-Suche über alle Spalten, dynamischen Kategorie-Filterchips, Spaltensortierung auf `<th>`, Farbfeld-Badges (`.farb-hex-pill`/`.farb-punkt`), einem Modal zum Hinzufügen von Zeilen und einem Zwei-Wege-Sync (`POST /api/sheet-sync`). Alles per JavaScriptCore und gegen den Klon `/tmp/vault-test` verifiziert.
